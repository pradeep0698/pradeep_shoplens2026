...
- `AI Analyzer / Analyze - GCS Video URI` and `Pub/Sub Worker / Simulate Pub/Sub Push Message` both reference a fixed placeholder GCS object (`gs://shoplens-dev-hls-segments/live/segment001.ts`) that doesn't actually exist in the bucket — a `502` from either is expected, not a regression, unless that object is replaced with a real one.
